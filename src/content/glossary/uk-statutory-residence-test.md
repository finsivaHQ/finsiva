---
term: "Statutory Residence Test (SRT)"
pronunciation: "/ˈstætʃ.ə.tər.i ˈrɛz.ɪ.dəns tɛst/"
category: "Tax Residency"
definition: "The Statutory Residence Test (SRT) is the legal framework used by HMRC to determine whether an individual is resident in the UK for tax purposes in any given tax year."
examples:
  - "If you spend 183 days or more in the UK during a tax year, you are automatically classified as a UK tax resident under the SRT."
relatedGuides:
  - "uk-expat-foreign-income-tax-guide"
relatedCalculators:
  - "united-kingdom-income-tax-income-tax-calculator"
relatedGlossary:
  - "uk-non-domicile-status"
  - "uk-remittance-basis"
---

The **Statutory Residence Test (SRT)** comprises three sequential tests:
1. **Automatic Overseas Tests:** Criteria where you are automatically non-UK resident (e.g. fewer than 16 days in the UK).
2. **Automatic UK Tests:** Criteria where you are automatically UK resident (e.g. 183+ days in the UK or only home in the UK).
3. **Sufficient Ties Test:** Evaluates UK ties (family, accommodation, work, 90-day tie) alongside days spent in the UK.

Explore residency impacts using our [UK Income Tax Calculator](/countries/united-kingdom/income-tax/income-tax-calculator/).
